...
observed process history. The charts in this package provide a comprehensive
set of measurements and visualizations that can be used to reason 
about the stability of flow process. 

We can use the results of the analysis to understand whether a process is stable, and 
...
operating modes for the process as it moves towards a stable states.

### `sample_path_flow_metrics.png`

Four-panel summary: The four charts are summarized at the top level in one
single chart.
...
At any point in time, the relationship  `L(T)=Λ(T)·w(T)` holds, so if `L(T)`
changes Little's Law invariant states that it must be because either `Λ(T)` or
`w(T)` or both changed.
...
- A combination of the two: since L(T) is the product of the two, if they move
  in the same direction they lead to apparently large changes in L(T)
- Conversely, when Λ(T) and w(T) move in opposite directions, they cancel each
  other and L(T) appears flat. This happens often when there is a feedback loop
  at play in the process.

These causal relationships allows us to monitor changes in L(T) and then
immediately investigate the cause of that change. See our
post [The Causal Arrow in Little's Law](https://www.polaris-flow-dispatch.com/i/171471652/the-causal-mechanism)
