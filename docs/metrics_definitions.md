...
From this we derive: 
- **Sample Path**
The *sample path* is the instantaneous number of items present in the system at time *t*:
**N(t) = A(t) − D(t), for t ∈ [0, T]**

- **Cumulative Presence H(T)**  
  The total “item-time” accumulated by all active items during [0, T]. This is the area under the sample path over [0,T].  
  **H(T) = ∫₀ᵀ N(t) dt**
...
- **`Λ(T)` — Cumulative Arrival Rate**  
  Let **A(T)** be the cumulative number of arrivals by time *T*. Then: 
  **Λ(T) = A(T) ⁄ T**

- **`w(T)` — Average Residence Time**  
  The average time items that *arrived over [0, T]* spend in the system. 
  **w(T) = H(T) ⁄ A(T)**
